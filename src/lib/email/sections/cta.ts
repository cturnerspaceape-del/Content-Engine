  ...
  return `
    <tr>
      <td style="padding:32px 32px 36px;font-family:Helvetica,Arial,sans-serif;text-align:center;">
        <a href="${escapeHtml(data.url)}" class="cta-button" style="display:inline-block;background:${PALETTE.accent};color:${PALETTE.onAccent};font-size:15px;font-weight:800;letter-spacing:0.06em;text-transform:uppercase;text-decoration:none;padding:16px 32px;border-radius:999px;mso-padding-alt:0;">${escapeHtml(data.label)}</a>
        ${
          data.supporting
